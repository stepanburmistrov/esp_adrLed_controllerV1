...
Этот репозиторий — готовый учебный проект для управления адресной лентой **WS2812B** на базе **ESP32**.
...
- Для продакшна задай корневой сертификат вместо `setInsecure()`.

## FAQ
- **status=3?** Это `WL_CONNECTED`.
- **Лаги UI при боте?** У нас бот в отдельной задаче — лагов нет.
- **Падает при старте?** Сервисы стартуют только после получения IP, чтобы избежать гонок.

## Лабы
1) Добавьте третью страницу палитры.  
2) Реализуйте эффект «дыхание» и добавьте кнопку в меню.  
3) Вынесите HTML/JS в SPIFFS и обслужите из файловой системы.

Подробнее про Алису — см. `yandex-alice.md`.
